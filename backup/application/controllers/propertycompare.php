...
class Propertycompare extends Frontend_Controller
{
    ...
    {    
        ...
        if(!empty($session_compare) && sw_count($this->session->userdata('property_compare'))>0):
                ...
                if(isset($json_obj->{"field_$key1"}))
                {
                    ...
                    if(!empty($row1)){
                        if($type=='CHECKBOX') {
                            ...
                            $property_compare['option_'.$key1]['tr'].='<td>'.$this->data['options_prefix_'.$key1].' '.$row1.' '.$this->data['options_suffix_'.$key1].'</td>';
                        }
                    } else {
