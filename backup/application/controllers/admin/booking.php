...
class Booking extends Admin_Controller
{
    ...
    public function index($property_id = 0, $pagination_offset=0)
	{
        ...
        $smart_search = '';
        /* get id by username */
        if(isset($_GET['smart_search']) && array_search($_GET['smart_search'], $this->data['users']) !== FALSE) {
            $user_id='';
        ...
        $config['total_rows'] = sw_count($this->data['reservations']);
        $config['per_page'] = 20;
    ...
    public function import_booked()
    {
		...
		else
		{
            ...
            if(sw_count($imported_counter) == 0)
    ...
    public function import_rate()
    {
		...
		else
		{
            ...
            foreach($properties as $property) {
                ...
                // Import all rates for property
                if(sw_count($data_rates) > 0)
                    $this->db->insert_batch('rates', $data_rates);
                
                // get all imports
                if(sw_count($data_rates_imports) > 0 && sw_count($data_rates) > 0)
                {
...
//        }        
        ...
        for ($month=6;$month<=9;$month++) :
                foreach ($cal_weeks[$month] as $key_week=>$week) {
                    foreach ($this->showroom_m->languages as $key => $value) {
                        $rules['rate_weekly_'.$month.'_'.$key_week.'_'.$key]= array ( 'field' => 'rate_weekly_'.$month.'_'.$key_week.'_'.$key,'label' => 'lang:Rate weekly '.$this->data['months'][$month].' day '.date('d',$week[0]).'/'.date('d', ($week[sw_count($week)-1]+86400)), 'rules' => 'trim|required|xss_clean' );
                    }
            }
        endfor;
       /* echo '<pre>';
    ...
    public function edit_withdrawal($id = NULL)
    {
        $this->load->model('withdrawal_m');
