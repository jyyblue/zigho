...
Widget-preview-image: /assets/img/widgets_preview/center_categories-slim.jpg
*/
?>
<?php
$CI = & get_instance();
...
<?php else: ?>
    ...
    <div class="properties properties-slim">
        <!-- PROPERTY LISTING -->
        ...
        ?>
            <div class="col-lg-4">
                <div class="property-card card  classfied-card">
                    <div class="property-card-header image-box">
                        <img src="<?php echo _simg($item['thumbnail_url'], '260x400', true); ?>" alt="" class="">
                    </div>
                    <div class="classfied-card-mask color-primary"></div>
                    <div class="property-card-box card-box classfied-card-box-title">
                        <h3 class="property-card-title">
                            <?php if(!empty($item['url'])) : ?>
                                <a  href='<?php _che($item['url']);?>'><?php _che($item['title_chlimit']); ?> <span class="item-count">(<?php _che($item['count']);?>)</span> </a>
                            <?php else: ?> 
                                <a  href='<?php echo site_url_nosuff($lang_code.'/'.$this->data['page_id'].'/?search={"v_search_option_'.$treefield_id.'":"'.rawurlencode($item['title_chlimit'].' - ').'"}'); ?>'><?php _che($item['title_chlimit']); ?> <span class="item-count">(<?php _che($item['count']);?>)</span> </a>
                            <?php endif;?>
                        </h3>
                    </div>
                    <div class="property-card-box card-box classfied-card-box-info">
                        <h3 class="property-card-title">
                            <?php if(!empty($item['url'])) : ?>
                    ...
                    <div class="property-card-box card-box classfied-card-box-category">
                        <div class="treefield-categories clearfix">
                            <?php if (sw_count($item['childs_8']) > 0) foreach ($item['childs_8'] as $child): ?>
                                <?php if(!empty($child['url'])): ?>
                                    <a href="<?php _che($child['url']); ?>"class="treefield-box-item btn-default text-color-primary"><?php _che($child['title']); ?> (<?php _che($child['count']);?>)</a>
                                <?php else:?>
                                    <a href='<?php echo site_url_nosuff($lang_code.'/'.$this->data['page_id'].'/?search={"v_search_option_'.$treefield_id.'":"'.rawurlencode($item['title_chlimit'].' - '.$child['title'].' - ').'"}'); ?>' class="treefield-box-item btn-default text-color-primary"><?php _che($child['title']); ?>  (<?php _che($child['count']);?>)</a>
                                <?php endif;?>
                            <?php endforeach; ?>
                        </div>
                    </div>
